...
test('set connection string in completely empty appsettings.json file', () => {
  var pathToTempConfigFile = createCopyOfSampleFile("completely-empty-sample-appsettings.json");
  ...
  assertConnectionStringValueIsNot(pathToTempConfigFile, expectedConnectionStringName, expectedConnectionStringValue);

  let temp = cp.execSync(`node ${systemUnderTest}`, options).toString();
  console.log(temp);

  assertConnectionStringValue(pathToTempConfigFile, expectedConnectionStringName, expectedConnectionStringValue);
})

test('set connection string in empty json appsettings.json file', () => {
  var pathToTempConfigFile = createCopyOfSampleFile("empty-json-sample-appsettings.json");
...
test('set new connection string in an appsettings.json file with existing connection strings', () => {
  ...
  assertConnectionStringValue(pathToTempConfigFile, 
    expectedConnectionStringName, expectedConnectionStringValue);
  assertConnectionStringValue(pathToTempConfigFile, 'connstr1', 'connstr1 value');
  assertConnectionStringValue(pathToTempConfigFile, 'connstr2', 'connstr2 value');
})
...
function assertConnectionStringValue(pathToTempConfigFile: string, expectedConnectionStringName: string, expectedConnectionStringValue: string) {
  const editor = new JsonEditor();
  editor.open(pathToTempConfigFile);

  expect(editor.getConnectionString(expectedConnectionStringName)).toBe(expectedConnectionStringValue);
}

function assertConnectionStringValueIsNot(pathToTempConfigFile: string, expectedConnectionStringName: string, expectedConnectionStringValue: string) {
  const editor = new JsonEditor();
  editor.open(pathToTempConfigFile);

  expect(editor.getConnectionString(expectedConnectionStringName)).not.toBe(expectedConnectionStringValue);
}
